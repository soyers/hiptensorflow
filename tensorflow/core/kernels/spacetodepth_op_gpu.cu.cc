...
__global__ void S2D(hipLaunchParm lp, const int32 nthreads, const dtype* input_ptr,
...
struct SpaceToDepthOpFunctor<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T, 4>::ConstTensor input,
    ...
    hipLaunchKernel(HIP_KERNEL_NAME(S2D), dim3(config.block_count), dim3(config.thread_per_block), 0, d.stream(), 
